...
router.use('/api/users', createProxyMiddleware({
  target: (process.env.USER_SERVICE_URL || 'http://localhost:3001').trim(),
  changeOrigin: true,
...
router.use('/api/inventory/public', createProxyMiddleware({
  target: (process.env.INVENTORY_SERVICE_URL || 'http://localhost:3002').trim(),
  changeOrigin: true,
...
router.use('/api/payments', authenticateToken, createProxyMiddleware({
  target: (process.env.PAYMENT_SERVICE_URL || 'http://localhost:3004').trim(),
  changeOrigin: true,
